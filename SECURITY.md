...
PoliteLoad is intended only for personal archival of content the logged-in user can already access.
...
PoliteLoad does not accept school credentials. Authentication happens in normal Chrome through the institution's SSO flow. Local browser profiles can still contain authenticated session material and must be treated as sensitive.
